...
export class ZmodemAddon implements ITerminalAddon {
  ...
  private trzszInit() {
    ...
    // TODO splice big data into small chunks
  ...
  private zmodemInit() {
    ...
    this.sentry = new Zmodem.Sentry({
      to_terminal: (octets: never) => writer(new Uint8Array(octets)),
      sender: (octets: string | Uint8Array) => sender(octets),
      on_retract: () => reset(),
  ...
  public sendFile(files: FileList) {
    ...
    Zmodem.Browser.send_files(session, files, {
      on_progress: (_: never, offer: never) => writeProgress(offer),
    })
      .then(() => session.close())
      .catch(() => this.reset())
  }
  ...
  private receiveFile() {
    ...
    session.on('offer', offer => {
      ...
      offer.accept().then(payloads => {
        const blob = new Blob(payloads as never, { type: 'application/octet-stream' })
